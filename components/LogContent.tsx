...
export class LogContent extends React.Component<any, LogContent> {
  ...
  private renderMessageContent = (message, name, id) => {
    if (message instanceof Array) return <LogContentArray key={`${name}${id}`} name={name} message={message} toggle={() => this.toggle(id)} isShow={this.state[id]} />
    if (typeof message === 'object' && message !== null) return <LogContentObject key={`${name}${id}`} name={name} message={message} toggle={() => this.toggle(id)} isShow={this.state[id]} />

    return <LogContentString name={name} value={message} />
  }

  private renderMessageChildren = (message, id) => {
    if (message instanceof Array && this.state[id]) return (
      <View style={defaultStyle.object} key={id}>
        {message.map((child, i) => this.renderMessage(child, i, `${ id }${i}`))}
      </View>
      )
    if (typeof message === 'object' && message && this.state[id]) return (
      <View style={defaultStyle.object} key={id}>
...
export const LogContentString = props => {
  ...
  const generateContent = () => {
    if (value === undefined) return <Text style={defaultStyle.valueUndefined}>undefined</Text>
     ...
     if (value === true || value === false) return <Text style={defaultStyle.valueBool}>{value.toString()}</Text>
     if (Number.isInteger(value)) return <Text style={defaultStyle.valueNumber}>{value}</Text>

    return <Text style={defaultStyle.valueString}>{`"${value.toString()}"`}</Text>
  }
...
export const LogContentObject = props => {
  ...
  useEffect(() => {
    ...
    Object.keys(message).forEach(key => {
      ...
      else if (typeof message[key] === 'string') msg += `${key}: "${message[key]}", `
      else msg += `${key}: ${String(message[key])}, `
    })
...
export const LogContentArray = props => {
  ...
  useEffect(() => {
    ...
    message.forEach(item => {
      ...
      else if (typeof item === 'string') msg += `"${item}", `
      else msg += `${String(item)}, `
    })
    setShowMsg(`${msg.length > 1 ? msg.slice(0, -2) : msg}]`)
  }, [])
